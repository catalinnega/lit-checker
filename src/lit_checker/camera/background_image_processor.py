...
class BackgroundImageProcessor:
    ...
    def __load_background_image(self, background_image_path: str) -> np.ndarray:
        frame = cv2.imread(background_image_path)
        self.log.info(f"Loaded background frame from {background_image_path}")
        return frame

    def __capture_background_image(self, url: str, background_image_path: str) -> np.ndarray:
