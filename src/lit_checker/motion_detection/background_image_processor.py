...
class BackgroundImageProcessor:
    ...
    def __load_background_image(self, background_image_path: str) -> NDArray[np.uint8]:
        ...
        frame = np.array(cv2_frame, dtype=np.uint8)
        self.log.info(f"Loaded background frame from {background_image_path}")
        return frame

    def __capture_background_image(
        self, url: str, background_image_path: str
    ) -> NDArray[np.uint8]:
        capture = cv2.VideoCapture(url)
        are_frames, cv2_frame = capture.read()
        if not are_frames:
            self.log.error("Frame could not be captured for background image")
    ...
    def __get_background_image_path(self, config: FilesConfig) -> str:
        return os.path.join(config.output_dir, config.background_image_fname)
