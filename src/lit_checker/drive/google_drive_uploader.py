...
class GoogleDriveUploader:
    def __init__(
        ...
        self.config = config
        self.authenticator = self.__login_with_service_account(
            config.client_json_file_path)
        ...
        self.folders_dict = self.init_gdrive_folders(
            ...
            folder_names=['motion_detections'])
    ...
    def upload_file_to_folder(self, file_path: str, folder_name: str) -> GoogleDrive | None:
        if folder_name in self.folders_dict:
            ...
            file.Upload()
            self.log.info(
                f"Uploaded at: {folder_file['title']}/{output_file_name}")
    ...
    def create_folder_if_not_present(self, folder_name: str, root_dir_id: str) -> GoogleDrive:
        ...
        if len(sub_files):
            self.log.info(
                f"Found sub_files in {root_dir_id}: {[f['title'] for f in sub_files]}")
        ...
        if folder_file is None:
            ...
            self.log.info(
                f"Created folder 'motion_detections' in {root_dir_id}")
        return folder_file

    def init_gdrive_folders(
            self,
            google_drive_root_folder: str,
            folder_names: list[str]) -> dict[str, GoogleDrive]:
            ...
            for folder_name in folder_names:
                if root_folder_file['title'] == root_directory:
                    folder_file = self.create_folder_if_not_present(
                        folder_name=folder_name,
                        root_dir_id=root_folder_file['id']
                    )
...
if __name__ == '__main__':
    ...
    gdrive_uploader.upload_file_to_folder(
        file_path=fpath, folder_name='motion_detections')
